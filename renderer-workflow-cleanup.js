(() => {
  function finalizeWorkflowLayout() {
    ...
    if (badge) {
      ...
      if (badgeText) badgeText.textContent = ' PF3 Project Reuse';
    }
    const footerLead = document.querySelector('footer span:first-child');
    if (footerLead) {
      footerLead.replaceChildren(createTextElement('b', '', 'V2.1 PF3:'), document.createTextNode(' Project Duplication / Reuse'));
    }
  }
